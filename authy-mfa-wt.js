...
return function (context, req, res) {
  ...
  async.series([
    ...
    function(callback) {
      if (req.method === 'GET' && !context.data.otp) {
        ...
        if(!req.query.token){
          res.writeHead(400)
          res.end('No authorization token found!')
        }
        //check that jwt is valid for this client
        jwt.verify(req.query.token, new Buffer(context.data.auth0_secret, 'base64'), function(err, decoded){
          if(err){
            res.writeHead(400)
  ...
  function renderOtpView(errors, sid, state) {
          ...
          <div class="modal-centrix">
            <div class="modal">
              <form onsubmit="showSpinner();" action="" method="GET" enctype="application/x-www-form-urlencoded">
                ...
                <div class="body">
                  <span class="description"  style="margin-top:2em;">Hi <strong><%- user || "" %></strong>, please enter your Authy code.</span>
    ...
    res.end(require('ejs').render(body, {
